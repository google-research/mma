...
URLS = {
  'svhn_extra': 'http://ufldl.stanford.edu/housenumbers/{}_32x32.mat',
...
def _load_svhn_extra():
  ...
  for split in ['train', 'test', 'extra']:
    with tempfile.NamedTemporaryFile() as f:
      request.urlretrieve(URLS['svhn_extra'].format(split), f.name)
    ...
    # svhn_extra raw data uses labels from 1 to 10; use 0 to 9 instead.
    dataset['labels'] -= 1
    splits[split] = dataset
  return splits


def _load_svhn_extra_combine_extra():
  splits = _load_svhn_extra()
  print(len(splits['train']['images']))
  splits['train']['images'] += splits['extra']['images']
  splits['train']['labels'] = np.concatenate((splits['train']['labels'], splits['extra']['labels']))
  del splits['extra']
  print(len(splits['train']['images']))
  print(splits.keys())
  return splits
...
def _load_mnist():
  ...
  for split, split_file in split_files:
    ...
    with tempfile.NamedTemporaryFile() as f:
      request.urlretrieve(
        URLS['mnist'].format(label_filename.format(split_file)), f.name)
      with gzip.GzipFile(fileobj=f, mode='r') as data:
...
def _load_fashionmnist():
  ...
  for split, split_file in split_files:
    with tempfile.NamedTemporaryFile() as f:
      request.urlretrieve(
        URLS['fashion_mnist'].format(image_filename.format(split_file)),
        f.name)
      with gzip.GzipFile(fileobj=f, mode='r') as data:
        assert _read32(data) == 2051
        n_images = _read32(data)
    ...
    with tempfile.NamedTemporaryFile() as f:
      request.urlretrieve(
        URLS['fashion_mnist'].format(label_filename.format(split_file)), f.name)
      with gzip.GzipFile(fileobj=f, mode='r') as data:
        assert _read32(data) == 2049
        n_labels = _read32(data)
        labels = np.frombuffer(data.read(n_labels), dtype=np.uint8)
...
CONFIGS = dict(
  ...
  cifar100=dict(loader=_load_cifar100,
                checksums=dict(train=None, test=None)),
  svhn=dict(loader=_load_svhn_extra,
            checksums=dict(train=None, test=None, extra=None)),
  svhn_extra=dict(loader=_load_svhn_extra_combine_extra,
                  checksums=dict(train=None, test=None, extra=None)),
)
